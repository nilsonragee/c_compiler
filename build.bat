...
IF %build_debug%==1 (
	...
	(
		msbuild "c_compiler.sln" -nologo -property:Configuration=Debug -property:Platform=x64 -target:Build
	...
	REM robocopy resources build\vs2019_c_compiler_Debug_x64\resources\ /MIR /NJH /NJS
...
IF %build_release%==1 (
	...
	SETLOCAL EnableDelayedExpansion
	(
		msbuild "c_compiler.sln" -nologo -property:Configuration=Release -property:Platform=x64 -target:Build
	...
	REM /NJH - No Job Header.
	REM /NJS - No Job Summary.
	REM robocopy resources build\vs2019_c_compiler_Release_x64\resources\ /MIR /NJH /NJS
